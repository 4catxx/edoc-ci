...
$sqlmain= "select * from schedule inner join doctor on schedule.docid=doctor.docid where schedule.scheduledate>='$today' AND status != 'Sukses' AND status != 'Dibatalkan' order by schedule.scheduledate asc";
$sqlpt1="";
$insertkey="";
$q='';
$searchtype="All";
if($this->input->post()){
    if(!empty($this->input->post("search"))){
        ...
        $sqlmain= "select * from schedule inner join doctor on schedule.docid=doctor.docid where schedule.scheduledate>='$today' AND status != 'Sukses' AND status != 'Dibatalkan' and (doctor.docname='$keyword' or doctor.docname like '$keyword%' or doctor.docname like '%$keyword' or doctor.docname like '%$keyword%' or schedule.title='$keyword' or schedule.title like '$keyword%' or schedule.title like '%$keyword' or schedule.title like '%$keyword%' or schedule.scheduledate like '$keyword%' or schedule.scheduledate like '%$keyword' or schedule.scheduledate like '%$keyword%' or schedule.scheduledate='$keyword' )  order by schedule.scheduledate asc";
        $insertkey=$keyword;
...
<tr >
<td width="13%" >
<a href="schedule" ><button  class="login-btn btn-primary-soft btn btn-icon-back"  style="padding-top:11px;padding-bottom:11px;margin-left:20px;width:125px"><font class="tn-in-text">Back</font></button></a>
</td>
<td >
...
for ($y=0;$y<$list11->num_rows();$y++){
    $row00=$list11->unbuffered_row('array');
    $d=$row00["docname"];
...
for ($y=0;$y<$list12->num_rows();$y++){
    $row00=$list12->unbuffered_row('array');
    $d=$row00["title"];
...
if($result->num_rows()==0){
    ...
    <br>
    <p class="heading-main12" style="margin-left: 45px;font-size:20px;color:rgb(49, 49, 49)">Kami tidak dapat menemukan apa pun yang terkait dengan kata kunci Anda.
    !</p>
    <a class="non-style-link" href= "'.base_url('patient').'/schedule"><button  class="login-btn btn-primary-soft btn"  style="display: flex;justify-content: center;align-items: center;margin-left:20px;">  Semua Sesi  </font></button>
    </a>
    </center>
    <br><br><br><br>
    </td>
    ...
    for ( $x=0; $x<($result->num_rows());$x++){
        ...
        for($q=0;$q<3;$q++){
            ...
            if ($status != 'Sukses' && $status != 'Dibatalkan') {
                echo '
                <td style="width: 25%;">
                        <div  class="dashboard-items search-items"  >
                            ...
                            <div style="width:100%">
                                    ...
                                    <a href="'.base_url('patient').'/booking?id='.$scheduleid.'" ><button  class="login-btn btn-primary-soft btn "  style="padding-top:11px;padding-bottom:11px;width:100%"><font class="tn-in-text">Pesan sekarang</font></button></a>
                            </div>
